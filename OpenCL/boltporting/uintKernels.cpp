//permuteAscInstantiated  yes

//permuteDescInstantiated
...
void sort4BitsAscending(u32 sortData[4], float sortVal[4], int startBit, int lIdx, __local u32* ldsSortData, __local float* ldsSortVal)
{
...
void permuteAscInstantiated(__global const u32* restrict gSrc, __global const u32* rHistogram, __global u32* restrict gDst, int4 cb, __global const float* restrict valueSrc, __global float* restrict valueDst)
{
	__local u32 ldsSortData[WG_SIZE*ELEMENTS_PER_WORK_ITEM + WG_SIZE];
	__local u32 localHistogramToCarry[NUM_BUCKET];
	__local u32 localHistogram[NUM_BUCKET * 2];
        __local float ldsSortVal[WG_SIZE*ELEMENTS_PER_WORK_ITEM+WG_SIZE];
	...
	int addr = blockSize*nBlocksPerWG*wgIdx + ELEMENTS_PER_WORK_ITEM*lIdx;
        ...
        #pragma unrool
	for (int iblock = 0; iblock<(((nBlocksPerWG) < (nBlocks)) ? (nBlocksPerWG) : (nBlocks)); iblock++, addr += blockSize)
	{
		u32 myHistogram = 0;

		u32 sortData[ELEMENTS_PER_WORK_ITEM];
		float sortVal[ELEMENTS_PER_WORK_ITEM];
		...
		{	
			...
			sortVal[i] = (addr + i < n) ? valueSrc[addr + i] : 0x0;
#else 
			sortData[i] = (addr + i < n) ? gSrc[addr + i] : 0xffffffff;
			sortVal[i] = (addr + i < n) ? valueSrc[addr + i] : 0xffffffff;
#endif 
		}
#else 

			sortData[i] = gSrc[addr + i];
			sortVal[i] = valueSrc[addr + i];
#endif 
#if defined(DESCENDING) 
		sort4BitsDescending(sortData, sortVal, startBit, lIdx, ldsSortData, ldsSortVal );
#else 
		sort4BitsAscending(sortData, sortVal, startBit, lIdx, ldsSortData, ldsSortVal);
#endif 
...

		{
			for (int ie = 0; ie<ELEMENTS_PER_WORK_ITEM; ie++)
			{
				...
				if (addr + ie < n)
					...
					valueDst[groupOffset + myIdx] = sortVal[ie];
			}
		}
